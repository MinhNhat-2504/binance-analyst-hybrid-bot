...
**Backup và sổ sách.** Stage paper mỗi sáng chạy `backup_state.py`: chép ledger, state, audit sqlite (qua backup API nên không rách file), kill-switch, đỉnh equity, canary, snapshots vào `backups/<ngày>/`, giữ 30 ngày. Thư mục đó nằm trên cùng ổ đĩa với repo, nên **thỉnh thoảng chép ra chỗ khác** (OneDrive, USB). `python export_income.py --env testnet --month 2026-09` kéo toàn bộ income của sàn (funding, phí, realized) thành một CSV mỗi tháng trong `reports/`; khi live thì chạy mỗi đầu tháng, đó là sổ sách. `python analyze_paper_attribution.py` dựng lại PnL paper theo từng đồng, đúng công thức của executor (tự kiểm: tổng theo đồng phải bằng ledger từng ngày), và in mức tập trung.
